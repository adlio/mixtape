...
impl AgentBuilder {
    ...
    /// let agent = Agent::builder()
    ///     .bedrock(ClaudeHaiku4_5)
    ///     .add_tools([Calculator, WeatherLookup, FileReader])
    ///     .build()
    ///     .await?;
    /// ```
    pub fn add_tools<T: Tool + 'static>(mut self, tools: impl IntoIterator<Item = T>) -> Self {
        for tool in tools {
            self.tools.push(box_tool(tool));
        }
        self
...
mod tests {
    ...
    fn test_builder_add_tools_multiple() {
        ...
        let builder = Agent::builder().add_tools(tools);
    ...
    fn test_builder_add_tools_empty() {
        ...
        let empty_tools: Vec<TestTool> = vec![];
        let builder = Agent::builder().add_tools(empty_tools);
    ...
    fn test_builder_add_tool_and_add_tools_chaining() {
        ...
        // Mix add_tool (single) with add_tools (multiple of same type)
        let builder = Agent::builder()
            .add_tool(Tool1)
            .add_tools(vec![Tool2, Tool2]);
