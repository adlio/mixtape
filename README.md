...
```rust
use mixtape::{Agent, ClaudeHaiku4_5, Result};
...
```toml
[dependencies]
mixtape = { version = "0.1", features = ["bedrock"] }
```
...
Most users want `bedrock` or `anthropic`. Add `mcp` for MCP server integration, `session` for conversation persistence.
...
| Crate | Purpose |
|-------|---------|
| **mixtape** | Core agent framework (add this to Cargo.toml) |
| **mixtape-tools** | Pre-built filesystem, process, and web tools |
| **mixtape-cli** | SQLite session storage and interactive REPL |
| **mixtape-anthropic-sdk** | Low-level Anthropic API client (used internally) |

Most projects need only `mixtape`. Add `mixtape-tools` for ready-to-use tools.

## Tools

Define tools with Rust types. Schemas generate automatically.

```rust
use mixtape::{Tool, ToolResult, ToolError};
...
The default `SlidingWindowConversationManager` is token-aware. It keeps recent messages that fit within the model's context limit, dropping older ones as needed.
...
```rust
use mixtape::mcp::{McpServerConfig, McpTransport};
...
Tools without a matching grant emit `PermissionRequired` events. See [`permissions.rs`](mixtape/examples/permissions.rs).
