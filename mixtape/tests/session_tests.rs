...
use mixtape::{Agent, SessionStore, Tool, ToolError, ToolResult};
...
use mixtape::{AgentEvent, AgentHook};
...
async fn test_session_resume_with_tool_history() {
    ...
    assert_eq!(session.messages[0].role, mixtape::MessageRole::User);
    assert_eq!(session.messages[1].role, mixtape::MessageRole::Assistant);

    // Second exchange
    assert_eq!(session.messages[2].role, mixtape::MessageRole::User);
    assert_eq!(session.messages[3].role, mixtape::MessageRole::Assistant);
}
